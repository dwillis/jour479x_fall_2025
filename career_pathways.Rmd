...
# EXERCISE: Students should complete this list with the appropriate position types
coaching_positions <- c(
  "Head Coach",
  "Interim Head Coach",
  "Associate Head Coach",
  "Assistant Coach",
  "___________",  # Fill in: What other coordinator role involves coaching?
  "___________",  # Fill in: Another coordinator role
  "___________"   # Fill in: Another coordinator role
)
...
head_coach_with_previous <- coaching_roles |>
  ...
  filter(next_position == "___________")  # EXERCISE: Fill in the position type
...
assistant_count <- previous_position_counts |>
  filter(position_title_standardized == "___________") |>  # EXERCISE: Fill in
  pull(n) |>
  sum()
...
**EXERCISE:** Based on the results above, write 2-3 sentences interpreting what this tells us about the typical career path to becoming a head coach. Which position is more common as a stepping stone?
...
**EXERCISE:** Why might recruiting experience be valuable (or not valuable) for someone aspiring to become a head coach? Write a brief response below based on the data and your understanding of coaching.

---

## Question 3: Common Career Pathways

**Research Question:** What are the most common career pathways to becoming a head coach?
...
### Common Patterns: Two-Step Pathways

Let's focus on the most direct pathways - those with exactly 2 positions ending in Head Coach:

```{r two-step-pathways}
# EXERCISE: Filter career sequences to only those with 2 positions
# (meaning one position before becoming head coach)
two_step_paths <- career_sequences |>
  filter(num_positions == ___)  # EXERCISE: Fill in the number

# Count and display
two_step_frequencies <- two_step_paths |>
  count(career_path, sort = TRUE)

cat("Coaches with exactly one previous position before head coach:",
    nrow(two_step_paths), "\n\n")
cat("Most common 2-step pathways:\n")
print(two_step_frequencies |> head(10), n = 10)
```

### Common Patterns: Three-Step Pathways

Now let's look at three-position careers:

```{r three-step-pathways}
# Filter to three-step pathways
three_step_paths <- career_sequences |>
  filter(num_positions == 3)

three_step_frequencies <- three_step_paths |>
  count(career_path, sort = TRUE)

cat("Coaches with exactly two previous positions before head coach:",
    nrow(three_step_paths), "\n\n")
cat("Most common 3-step pathways:\n")
print(three_step_frequencies |> head(10), n = 10)
```
...
Based on this analysis, we can identify several key patterns in coaching career pathways:
...
## Extension Exercise: Your Own Analysis

**EXERCISE:** Choose one of the following questions to explore with additional code:

1. Do career pathways differ by conference or school type?
2. How has the typical career pathway changed over time (comparing coaches who became head coaches in different decades)?
3. What's the average number of years spent in each position type before advancing?
4. Are there differences in career pathways for coaches at Power 5 vs mid-major programs?

Add your code and analysis below:

```{r extension-analysis}
# Your code here
